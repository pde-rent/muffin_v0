		...
		case '%':
		{
			...
			case 'A': // weekday name
				...
				for (size_t i = 0; i < 7; ++i)
				{
					...
					if (!strncmp(strp_weekdays[i], s, len))
					{
						tm->tm_wday = i;
						s += len;
						working = true;
					...
					else if (!strncmp(strp_weekdays[i], s, 3))
					{
						tm->tm_wday = i;
						s += 3;
						working = true;
			...
			case 'h': // month name
				...
				for (size_t i = 0; i < 12; ++i)
				{
					...
					if (!strncmp(strp_monthnames[i], s, len))
					{
						tm->tm_mon = i;
						s += len;
						working = true;
					...
					else if (!strncmp(strp_monthnames[i], s, 3))
					{
						tm->tm_mon = i;
						s += 3;
						working = true;
