...
/*	static	size_t		timestamp;
	...
	int		load_all(string file_name, t_tick_data *data)
	{
		string			tmp;
			...
			while (fread(buf, 1, BUFF_SIZE, histo)) //1 = sizeof(char)
			{
				...
				while (buf[i] && (j < nb_ticks) && (j < TICK_BUFF_SIZE)) // is ascii >> not null nor lost
				{
					if (chunk == 3)
					{
						...
						{
							tmp[++l] = '\0';
							data->epoch->push_front((size_t)fast::parse_zu(tmp));
							l = -1;
							chunk = 1;
							i++;
						}
					...
					if (chunk == 1)
					{
						...
						{
							tmp[++l] = '\0';
							data->px->push_front((float)fast::parse_f(tmp));
							l = -1;
							chunk = 2;
							i++;
						}
					...
					if (chunk == 2)
					{
						...
						{
							tmp[++l] = '\0';
							data->vol->push_front((float)fast::parse_f(tmp));
							l = -1;
							chunk = 3;
							i++;
							j++; //the whole tick data is now readable
							trigger::tick(data);
							trigger::circular(data);
						}
					}
