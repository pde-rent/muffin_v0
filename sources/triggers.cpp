...
namespace trigger
{
	...
	namespace	trade
	{
			...
			std::string	comment)		//unique comment to log
		{
			...
			*/
			for (std::deque<t_open_order>::iterator i = env->order_book->begin(); i != env->order_book->end(); ++i)
				if ((*i).ticket == ticket)
				{
					closed = *i;
					env->order_book->erase(i);
			...
			archive.close_price = (closed.type > 0 ? (TICK(0) - SPREAD) : (TICK(0) + SPREAD));
			...
			archive.profit = (archive.close_price - archive.entry_price) * archive.type * archive.entry_price;
			//archive.order_max_runup = closed.order_max_runup;
			//archive.order_max_dd = closed.order_max_dd;
			env->acc->balance += archive.profit;
